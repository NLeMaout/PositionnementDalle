...
NbEpochs=500
NbLayerHidden=4
FeatureAllreadyDone=False
for NbCellLayer in [700]:
	ActivationType="elu" #data value between -1 and 1
	BATCH_SIZE = 32
	Optimizer='adadelta' #'adam'
	# To avoid overfitting
	LayerDropout=False #True
	LayerDropoutValue=0.1
	L2Regularization=False #True
	L2RegularizationValue=0.001
	# For plot curve name info only (how input has been generated) No infuence here
	NbDalleVoisineMax=15
	NormalIndicator=True
	# Running type
	restart=True # loading weight from previous study
	optimization=True
	# Name of file containing features
	input_file='inputv5_between-1and1_Max6Min-6_noshuffle_normalv3.txt'
	target_file='targetv5_between-1and1_Max6Min-6_noshuffle_normalv3.txt'
	#### SETING-UP ######

	if FeatureAllreadyDone==False:
	...
	if restart and os.path.exists('model05_'+str(NbLayerHidden)+'_'+str(NbCellLayer)+'_'+str(LayerDropout)+'.h5'):
		try:
			import json
			model.load_weights('model05_'+str(NbLayerHidden)+'_'+str(NbCellLayer)+'_'+str(LayerDropout)+'.h5')
		except:
	...
	if optimization:
		...
		if SaveWeight:
			import json
			model.save('model05_'+str(NbLayerHidden)+'_'+str(NbCellLayer)+'_'+str(LayerDropout)+'.h5')
	...
	for i in range(len(vrai[0])):
		...
		#if abs(modelresult[0][i]-entree[0][i])>abs(modelresult[0][i]-vrai[0][i]):
		if abs(entree[0][i]-vrai[0][i])<abs(modelresult[0][i]-vrai[0][i]) and abs(entree[0][i]-modelresult[0][i])<abs(modelresult[0][i]-vrai[0][i]):
		...
		if abs(Delta)<10:
			...
			if CorrectDirection:
				...
				print('\t :( :( :( Delta=',Delta,'% Very far from true response and in bad direction!')
		elif abs(Delta)>=75.:
			if CorrectDirection:
